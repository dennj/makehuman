...
from .body_handler.humanstate import HumanState
from .body_handler.modifiergroups import ModifierInfo
from .body_handler.randomizationsettings import RandomizationSettings

mhapi: gui3d.app.mhapi = gui3d.app.mhapi
...
from .body_handler.body_handler import BodyHandler, ImageFormat, Sides
from .body_handler import mh2opengl
...
class MassProduceTaskView(gui3d.TaskView):
    ...
    def __init__(self, category):
        ...
        self._body_handler = BodyHandler()
    ...
    def _setupRightPanel(self, r):
        ...
        self.addRightWidget(self._createRenderSettings(r))
        self.addRightWidget(mhapi.ui.createLabel())
        self.addRightWidget(self._createProducePanel(r))
        self.addRightWidget(self._createRotationPanel(r))
    ...
    def _setupRandomizeProxies(self, layout, r):
        ...
        layout.addWidget(r.addUI("proxies", "upperClothes",
                                 mhapi.ui.createCheckBox(label="Randomize upper body clothes", selected=False)))
        layout.addWidget(r.addUI("proxies", "lowerClothes",
                                 mhapi.ui.createCheckBox(label="Randomize lower body clothes", selected=False)))
    ...
    def _setupAllowedSkinsTables(self, layout, r):
        ...
        for n in skinBaseNames:
            ...
            if self._matchesEthnicGender(n, "female") and not "special" in n:
                ...
                if self._matchesEthnicGender(n, ethnicity="asian") and not self._matchesEthnicGender(n,
                                                                                                     ethnicity="caucasian"):
                    female["asian"].setChecked(True)
                if self._matchesEthnicGender(n, ethnicity="caucasian"):
                    female["caucasian"].setChecked(True)

            if self._matchesEthnicGender(n, "male") and not self._matchesEthnicGender(n,
                                                                                      "female") and not "special" in n:
    ...
    def _createRenderSettings(self, r):
        self.render_settings_panel = mhapi.ui.createGroupBox("Render settings")
        self.render_settings_panel.addWidget(mhapi.ui.createLabel("Front delta x"))
        r.addUI("render", "frontDx",
                self.render_settings_panel.addWidget(mhapi.ui.createTextEdit("30")))
        self.render_settings_panel.addWidget(mhapi.ui.createLabel("Front delta y"))
        r.addUI("render", "frontDy",
                self.render_settings_panel.addWidget(mhapi.ui.createTextEdit("30")))
        self.render_settings_panel.addWidget(mhapi.ui.createLabel("Front delta z"))
        r.addUI("render", "frontDz",
                self.render_settings_panel.addWidget(mhapi.ui.createTextEdit("30")))
        self.render_settings_panel.addWidget(mhapi.ui.createLabel("Side delta x"))
        r.addUI("render", "sideDx",
                self.render_settings_panel.addWidget(mhapi.ui.createTextEdit("30")))
        self.render_settings_panel.addWidget(mhapi.ui.createLabel("Side delta y"))
        r.addUI("render", "sideDy",
                self.render_settings_panel.addWidget(mhapi.ui.createTextEdit("30")))
        self.render_settings_panel.addWidget(mhapi.ui.createLabel("Side delta z"))
        r.addUI("render", "sideDz",
                self.render_settings_panel.addWidget(mhapi.ui.createTextEdit("30")))

        res_options = ["800x800", "1280x1280", "1920x1920", "2560x2560"]
        self.render_settings_panel.addWidget(mhapi.ui.createLabel("Resolution"))
        r.addUI("render", "resolution",
                self.render_settings_panel.addWidget(mhapi.ui.createComboBox(data=res_options)))

        return self.render_settings_panel

    def _createProducePanel(self, r):
        self.producePanel = mhapi.ui.createGroupBox("Render & Produce")
        ...
        self.produceButton = self.producePanel.addWidget(mhapi.ui.createButton("Execute"))
    ...
    def _createRotationPanel(self, r):
        self.rotationPanel = mhapi.ui.createGroupBox("rorate")

        self.rotateButton = self.rotationPanel.addWidget(mhapi.ui.createButton("Rotate"))

        @self.rotateButton.mhEvent
        def onClicked(event):
            self._onRotateClick()

        return self.rotationPanel
    ...
    def _createModelingSettings(self, r):
        ...
        r.addUI("modeling", "maxdev", self.modelingPanel.addWidget(
            mhapi.ui.createSlider(value=0.3, min=0.0, max=1.0, label="Max deviation from default")))
    ...
    def _createMacroSettings(self, r):
        ...
        r.addUI("macro", "ageMinimum",
                self.macroPanel.addWidget(mhapi.ui.createSlider(label="Minimum age", value=0.45)))
        r.addUI("macro", "ageMaximum",
                self.macroPanel.addWidget(mhapi.ui.createSlider(label="Maximum age", value=0.95)))
        ...
        r.addUI("macro", "weightMinimum",
                self.macroPanel.addWidget(mhapi.ui.createSlider(label="Minimum weight", value=0.1)))
        r.addUI("macro", "weightMaximum",
                self.macroPanel.addWidget(mhapi.ui.createSlider(label="Maximum weight", value=0.9)))
        ...
        r.addUI("macro", "heightMinimum",
                self.macroPanel.addWidget(mhapi.ui.createSlider(label="Minimum height", value=0.2)))
        r.addUI("macro", "heightMaximum",
                self.macroPanel.addWidget(mhapi.ui.createSlider(label="Maximum height", value=0.9)))
        ...
        r.addUI("macro", "muscleMinimum",
                self.macroPanel.addWidget(mhapi.ui.createSlider(label="Minimum muscle", value=0.3)))
        r.addUI("macro", "muscleMaximum",
                self.macroPanel.addWidget(mhapi.ui.createSlider(label="Maximum muscle", value=0.8)))
        ...
        r.addUI("macro", "ethnicityabsolute",
                self.macroPanel.addWidget(mhapi.ui.createCheckBox(label="Absolute ethnicity", selected=True)))
    ...
    def _onProduceClick(self):
        ...
        settings['AA'] = True
        settings['dimensions'] = self.randomizationSettings.getValue("render", "resolution")\
            .split('x')

        settings['lightmapSSS'] = True
        ...
        while i > 0:
            ...
            self._body_handler.set_settings(self.randomizationSettings)
            ...
            self._body_handler.produce(name, format)
            ...
            # render Front side
            self._body_handler.set_side(Sides.FRONT)
            self._body_handler.apply_random_angle("front")
            # self._body_handler.apply_random_zoom()
            self._body_handler.render(settings, name + '_front', ImageFormat.PNG)

            # render Left side
            self._body_handler.set_side(Sides.LEFT)
            self._body_handler.apply_random_angle("side")
            # self._body_handler.apply_random_zoom()
            self._body_handler.render(settings, name + '_left', ImageFormat.PNG)
