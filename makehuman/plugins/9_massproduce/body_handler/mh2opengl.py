...
def Render(settings, filename, fileType):
    ...
    if not mh.hasRenderToRenderbuffer():
        ...
        width = int(settings['dimensions'][0])
        height = int(settings['dimensions'][1])
        ...
        if settings['AA']:
            ...
            img = img.resized(width/2, height/2, filter=pyimg.FILTER_BILINEAR)
    ...
    gui3d.app.getCategory('Rendering').getTaskByName('Viewer').save_image(filename)

    # image = gui.ZoomableImageView()
    #
    # if not os.path.exists(mh.getPath('render')):
    #     os.makedirs(mh.getPath('render'))
    # path = mh.getPath('render')
    # # filename, ftype = mh.getSaveFileName(os.path.splitext(path)[0],
